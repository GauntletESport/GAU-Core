...
CzGAUWallet::CzGAUWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgau seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGAUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGAUSeed_deprecated()) {
                    LogPrintf("%s: Updated zGAU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgau seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgau wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGAUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGAUCount(nCountLastUsed);
    else if (!walletdb.ReadZGAUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGAUWallet::Lock()
{
    seedMaster = 0;
}

void CzGAUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGAUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGAU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGAUWallet::LoadMintPoolFromDB()
{
...
void CzGAUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGAUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGAUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgauTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGAUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGAU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgauTracker which also adds to database
    pwalletMain->zgauTracker->Add(dMint, true);
...
void CzGAUWallet::SeedToZGAU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGAUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGAUWallet::GenerateDeterministicZGAU(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGAUWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGAUWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzGAUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
