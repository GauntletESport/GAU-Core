...
CzGAUTracker::CzGAUTracker(std::string strWalletFile)
{
...
CzGAUTracker::~CzGAUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGAUTracker::Init()
{
...
bool CzGAUTracker::Archive(CMintMeta& meta)
{
...
bool CzGAUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGAUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGAUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGAUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzGAUTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzGAUTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzGAUTracker::GetSerialHashes()
{
...
CAmount CzGAUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGAUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGAUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGAUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGAUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGAUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGAUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGAUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGAUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGAUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGAUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzGAUWallet* zGAUWallet)
{
    bool iszGAUWalletInitialized = (NULL != zGAUWallet);
    CMintMeta meta;
    ...
    if (! iszGAUWalletInitialized)
        zGAUWallet = new CzGAUWallet(strWalletFile);
    meta.isSeedCorrect = zGAUWallet->CheckSeed(dMint);
    if (! iszGAUWalletInitialized)
        delete zGAUWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzGAUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGAUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGAUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGAUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGAUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGAUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzGAUWallet* zGAUWallet = new CzGAUWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zGAUWallet);
        }
        delete zGAUWallet;
        LogPrint("zero", "%s: added %d dzgau from DB\n", __func__, listDeterministicDB.size());
    }
