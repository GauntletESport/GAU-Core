...
public:
    ...
    bool WriteZGAUSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGAUSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGAUSeed_deprecated(uint256& seed);
    bool EraseZGAUSeed();
    bool EraseZGAUSeed_deprecated();

    bool WriteZGAUCount(const uint32_t& nCount);
    bool ReadZGAUCount(uint32_t& nCount);
