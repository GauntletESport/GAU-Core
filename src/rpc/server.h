...
/**
 * Gauntlet RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZgauSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzgauseed(const UniValue& params, bool fHelp);
extern UniValue getzgauseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzgau(const UniValue& params, bool fHelp);
extern UniValue dzgaustate(const UniValue& params, bool fHelp);
