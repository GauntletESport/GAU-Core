...
// The zGAU block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zGAU is a hash of the serial
...
{
    //Create an output returning the zGAU that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZGAUOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zGAU output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zGAU", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZGAUOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zGAU output", __func__);
...
{
    CzGAUTracker* zgauTracker = pwallet->zgauTracker.get();
    CMintMeta meta;
    if (!zgauTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zgauTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a GAU stake is the outpoint
