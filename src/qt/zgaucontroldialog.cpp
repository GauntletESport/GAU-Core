...
#include "ui_zgaucontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zgauTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zgauTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zGAU.";
            else if (!mint.isSeedCorrect)
                strReason = "The zGAU seed used to mint this zGAU is not the same as currently hold in the wallet";
            else
