...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZGAUModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zgauTracker->HasMintTx(hash))) {
            //zGAU stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZGAU;
...
bool IsZGAUType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZGAU:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZGAU || type == TransactionRecord::MNReward) {
